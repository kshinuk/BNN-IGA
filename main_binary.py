...
parser.add_argument('--model', '-a', metavar='MODEL', default='resnet_binary', choices=model_names,
                    help='model architecture: ' + ' | '.join(model_names) + ' (default: alexnet)')
parser.add_argument('--input_size', type=int, default=None,
                    help='image input size')
parser.add_argument('--ao_bit', type=float, default=1,
                    help='activation out quantize bit')
parser.add_argument('--w_bit', type=float, default=1,
                    help='weight quantize bit')
parser.add_argument('--adc_bit', type=int, default=1,
                    help='analog digital convert quantize bit')
...
def main():
    ...
    if args.evaluate:
        ...
        setup_logging(os.path.join(save_path, 'log_train.txt'))
    ...
    model_config = {'input_size':args.input_size, 'dataset':args.dataset, 
                    'ao_bit':args.ao_bit, 'w_bit':args.w_bit ,'adc_bit':args.adc_bit}
...
#                                           'weight_decay': args.weight_decay}})
    ...
    #dataset load
    train_dataset, val_dataset = get_dataset(args.dataset, 'train', transform['train'])
