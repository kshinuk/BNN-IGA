...
def adc(PS, minimum = -128, maximum = 128, bit = 1):
    '''
    PS is patial sum and PS_q is quantized patial sum.
...
def impulse_gradient_approximation(x, minimum=-1, maximum=1, sigma=0.05, bit=1):
    '''
    this function is impulse_gradient_approximation (IGA) which approximates gradient of quantization function
...
class ActivationOutQuantF(Function):
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.ao_bit == 1.5:
            ...
            quant_grad = impulse_gradient_approximation(x=input, minimum=-1, maximum=1, sigma=1.1, bit=ctx.ao_bit)
...
class WeightQuantF(Function):
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.w_bit==1.5:
            ...
            quant_grad = impulse_gradient_approximation(x=input, minimum=-1, maximum=1, sigma=1.1, bit=ctx.w_bit)
        grad_input = grad_output.clone() * quant_grad
        return grad_input, None
...
class PartialSumQuantF(Function):
    ...
    def backward(ctx, grad_output):
        ...
        if 4 < ctx.adc_bit: # ADC not applied
            ...
            sigma      = [6,8,32,32] # e.g., sigma values at 1bit, 2bit, 3bit and 4bit. each values can be tuned.
...
class ActivationOutBinarizeF(Function):
    @staticmethod
    ...
    def backward(ctx, grad_output):
        input, = ctx.saved_tensors
        grad_input = grad_output.clone() # Identity
        #grad_input[input.abs() > 1] = 0 # Clip
        return grad_input

BinarizeInput = ActivationOutBinarizeF.apply


class WeightBinarizeF(Function):
    @staticmethod
    def forward(ctx, input):
        ctx.save_for_backward(input)
        output = input.new(input.size())
...
BinarizeWeight = WeightBinarizeF.apply
...
class BinarizeConv2d(nn.Conv2d):
    ...
    def forward(self, input):
        ...
        out = 0
        if input.size(1) != 3:
            quantized_input = input.clone()
            quantized_weight = self.weight.clone()
            #quantized_input      = QuantInput(input.data,        self.ao_bit)
            #quantized_weight     = QuantWeight(self.weight.data, self.w_bit)
            quantized_input      = BinarizeInput(quantized_input)
            quantized_weight     = BinarizeWeight(quantized_weight)
            CiM_row              = 128
            channelSliceWord     = int(CiM_row/9)
            divided_inputTensor  = list(torch.split(quantized_input, channelSliceWord, dim=1))
            divided_weightTensor = list(torch.split(quantized_weight, channelSliceWord, dim=1))
...
class BinarizeLinear(nn.Linear): 
    ...
    def forward(self, input):
        ...
        if not self.is_last_layer:
            ...
            input_list  = torch.split(quantized_input, self.CiM_row, dim=1)
            weight_list = torch.split(quantized_weight, self.CiM_row, dim=1)
            for inputIter, weightIter in zip(input_list, weight_list):
                partial_sum           = nn.functional.linear(inputIter, weightIter)
                #partial_sum_quantized = QuantSum(partial_sum, self.adc_bit)
                out += partial_sum
